...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { teamMembershipId, profileId, deleteAuthUser = true }: DeleteUserRequest = await req.json();

    if (!teamMembershipId && !profileId) {
      return new Response(
        JSON.stringify({ error: "Either teamMembershipId or profileId is required" }),
        { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } }
      );
    }

    console.log("Deleting user:", { teamMembershipId, profileId });
